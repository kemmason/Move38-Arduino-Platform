...
/*
    ...
    Fade Effect - A Fade from one color to another

*/

struct FadeEffect_t : Effect_t {

    Color m_fromColor;
    Color m_toColor;

    uint16_t m_duration_ms;
    ...
    void nextStep() {
      ...
      // linear interpolation between the fromColor and toColor
      byte from_r = GET_5BIT_R(m_fromColor);
      byte from_g = GET_5BIT_G(m_fromColor);
      byte from_b = GET_5BIT_B(m_fromColor);

      byte to_r = GET_5BIT_R(m_toColor);
      byte to_g = GET_5BIT_G(m_toColor);
      byte to_b = GET_5BIT_B(m_toColor);

      uint16_t timeLeft = endTime - now;

      byte new_r = (from_r * timeLeft + to_r * ( m_duration_ms - timeLeft)) / m_duration_ms;
      byte new_g = (from_g * timeLeft + to_g * ( m_duration_ms - timeLeft)) / m_duration_ms;
      byte new_b = (from_b * timeLeft + to_b * ( m_duration_ms - timeLeft)) / m_duration_ms;

      Color displayColor = makeColorRGB(new_r, new_g, new_b);
    ...
    void start( Color fromColor, Color toColor, uint16_t duration_ms ) {
...
static FadeEffect_t fadeEffect;

void fadeTo( Color fromColor, Color toColor, uint16_t duration_ms) {
  clearEffects();
  fadeEffect.start( fromColor, toColor, duration_ms );
}
